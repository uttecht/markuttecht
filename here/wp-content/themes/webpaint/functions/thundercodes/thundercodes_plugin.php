	        ...
	        switch (n) {
	            case 'thundercodes_button':
	                ...
	                c.onRenderMenu.add(function(c, m) {
	                    ...
	                    m.add({title : 'Latest Projects Slider', onclick : function() {
	                    	tinyMCE.activeEditor.execCommand('mceInsertContent', 0, '[latest_projects_slider title="YOUR_TITLE" cat_slugs="CATEGORY_SLUGS" number=12]');
	                    }});
	                    
	                     m.add({title : 'Latest Projects Carousel', onclick : function() {
	                    	tinyMCE.activeEditor.execCommand('mceInsertContent', 0, '[latest_projects_carousel title="YOUR_TITLE" cat_slugs="CATEGORY_SLUGS" number=12]');
	                    }});
	                    ...
	                    m.add({title : 'Twitter', onclick : function() {
	                    	tinyMCE.activeEditor.execCommand('mceInsertContent', 0, '[twitter user="TWITTER_ACCOUNT" number="3"]');
	                    }});
	                    
	                    m.add({title : 'Facebook Page', onclick : function() {
	                    	tinyMCE.activeEditor.execCommand('mceInsertContent', 0, '[facebook_page url="URL_FACEBOOK_PAGE"]');
	                    }});

	                    m.add({title : 'Dribbble', onclick : function() {
	                    	tinyMCE.activeEditor.execCommand('mceInsertContent', 0, '[dribbble user="DRIBBBLE_USER" number=20]');
	                    }});
	                    
	                    m.add({title : 'flickr', onclick : function() {
	                    	tinyMCE.activeEditor.execCommand('mceInsertContent', 0, '[flickr user_id="FLICKRUSER@N00" number=15]');
	                    }});
	                    
	                    m.add({title : 'Portfolio Info', onclick : function() {
	                    	tinyMCE.activeEditor.execCommand('mceInsertContent', 0, '[portfolio_info_set]<br>[portfolio_info title="Date:"]<?php echo date(get_option("date_format")); ?>[/portfolio_info]<br>[portfolio_info title="Categories:"]Category 1, Category 2[/portfolio_info]<br>[portfolio_info title="Client:"]ThunderBuddies[/portfolio_info]<br>[portfolio_info title="Link:"]http://themes.thunderbuddies4life.com[/portfolio_info][/portfolio_info_set]');   
	                    }});
		...
		switch (thundershortcode) {
		    case "button": 
		    				var form = jQuery('<div id="thundercodes-form" class="thundercodes-form"><table id="thundercodes-table" class="form-table">\
										...
										<?php thundercodes_select('color','Color',array('blue','pink','purple','black','green','red','brown','yellow','aqua','orange'),'Select the hover Color');?>
		    ...
		    case "tabs": 
		    				var form = jQuery('<div id="thundercodes-form" class="thundercodes-form"><table id="thundercodes-table" class="form-table">\
										...
										<?php thundercodes_select('align','Tabs Align',array('side','top'),'Display the tabs bound to the left or on top (classic)');?>
										</table>\
										<p class="submit">\
