	...
	{			
		...
		{				
			...
			//Next/Prev Links
				if($current<$total){
					echo '<li>
					<a href="'.get_pagenum_link($current+1).'">'.__("next","tb_webpaint").'</a>
					</li>
					';
					echo '<li>
					<a href="'.get_pagenum_link($total).'">'.__("last","tb_webpaint").'</a>
					</li>
					';
				}
				else{
					echo '<li>
					<a href="'.get_pagenum_link(1).'">'.__("first","tb_webpaint").'</a>
					</li>
					';
					echo '<li>
					<a href="#" class="current">'.__("last","tb_webpaint").'</a>
					</li>
					';
				}
			// The End
...
function head_pagination($paged,$posts_per_page)
{
	...
	{			
			...
			if($total > 1){				 
					...
					switch($paged){
						case 1:
								echo '<a href="'.get_pagenum_link($total).'" title="'.__("Previous Page","tb_webpaint").'"><i class="icon-left-open-1"></i></a><a href="'.get_pagenum_link($paged+1).'" title="'.__("Next Page","tb_webpaint").'"><i class="icon-right-open-1"></i></a>';
								break;
						case $total:
								echo '<a href="'.get_pagenum_link($paged-1).'" title="'.__("Previous Page","tb_webpaint").'"><i class="icon-left-open-1"></i></a><a href="'.get_pagenum_link(1).'" title="'.__("Next Page","tb_webpaint").'"><i class="icon-right-open-1"></i></a>';
								break;
						default:
								echo '<a href="'.get_pagenum_link($paged-1).'" title="'.__("Previous Page","tb_webpaint").'"><i class="icon-left-open-1"></i></a><a href="'.get_pagenum_link($paged+1).'" title="'.__("Next Page","tb_webpaint").'"><i class="icon-right-open-1"></i></a>';
								break;
					}
					echo '</div>';
			}
